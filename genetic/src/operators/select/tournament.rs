...
impl TournamentSelector {
    ...
    fn choose<T: Copy, R: Rng>(&mut self, vec: Vec<T>, amt: usize, rng: &mut R) -> Vec<T> {
        ...
        let indices = indices
            .choose_multiple_weighted(rng, amt, |i| weights[*i])
            .unwrap();
...
impl SelectOperator for TournamentSelector {
    fn select_from<P: crate::population::Population, R: Rng>(
        ...
        for _ in 0..self.num_matings {
            let mut participants: Vec<usize> = sample(rng, population.len(), self.tournament_size)
                .into_iter()
                .collect();
            participants.sort();
            ...
            let parents = {
                if self.remove_selected {
                    parent_indices
                        .into_iter()
                        .map(|i| population.remove(i).into_genotype())
                        .collect()
                } else {
                    parent_indices
                        .into_iter()
                        .map(|i| (*population[i].genotype()).clone())
                        .collect()
                }
            };
...
pub mod tests {
    use super::*;
    use crate::{genetic::*, individual::IndividualStruct};
    ...
    fn test_select_from_noremove() {
        ...
        let candidates = vec![
            29, 71, 95, 87, 97, 0, 35, 25, 53, 91, 97, 43, 99, 100, 28, 92, 40, 45, 43, 5,
        ];

        let evaluated: Vec<IndividualStruct<u64, u64>> = candidates
            .into_iter()
            .map(|x| IndividualStruct::new(x, x, NotNan::new(0.).unwrap()))
            .collect();
