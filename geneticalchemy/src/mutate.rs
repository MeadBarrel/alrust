...
use genetic::{error::Result, op::MutateOperator};

use crate::genome::AlchemyGenome;
...
impl AlchemyMutator {
    ...
    fn mutate_ingredients<R: Rng>(&mut self, genome: &mut AlchemyGenome, rng: &mut R) {
        let indices_to_mutate = (0..genome.len()).choose_multiple(rng, self.num_mutations_ing);
        for index in indices_to_mutate {
            let new_ingredient = rng.gen_range(0..self.grimoire_size);
            if genome.iter().any(|x| x.ingredient_index == new_ingredient) {
                continue;
            }
    ...
    fn mutate_amounts<R: Rng>(&mut self, genome: &mut AlchemyGenome, rng: &mut R) {
        let genes_to_mutate = genome
            .iter_mut()
            .choose_multiple(rng, self.num_mutations_amt);
        for gene in genes_to_mutate {
            ...
            if reverse {
                gene.amount = gene.amount - min(gene.amount, delta)
            } else {
                gene.amount += delta
            }
        }
