...
use crate::{optimized::*, theoretical::*, types::*};
...
pub fn mix_volume(mix: &Mix) -> f64 {
    let w: u64 = mix
        .ingredients
        .iter()
        .map(|(ingredient, count)| ingredient.alchemical_weight as u64 * count.to_owned() as u64)
        .sum();
    (w as f64 - 1.) / 10.
}
...
pub fn mix_effect(mix: &Mix, property: Property) -> Theoretical<f64> {
    ...
    for (ingredient, count) in &mix.ingredients {
        multiplier = multiplier
            * (Theoretical::from(1.)
                + Theoretical::from(ingredient.modifiers[property as usize].multiplier)
...
mod tests {
    ...
    fn create_compendium() -> data::Compendium {
        let ingredients = vec![
            data::Ingredient::new(
                ...
                1,
                "Herbology",
                vec![(Property::DirectHealing, Modifier::new(Some(1.2), Some(0.0)))]
                    .into_iter()
                    .collect(),
            ),
            data::Ingredient::new(
                ...
                vec![
                    ...
                    (
                        ...
                        Modifier::new(Some(0.979), Some(-0.75)),
                    ),
                ]
                .into_iter()
                .collect(),
            ...
            data::Ingredient::new(
                ...
                0,
                "Petrology",
                vec![(Property::DirectHealing, Modifier::new(Some(0.), Some(0.96)))]
                    .into_iter()
                    .collect(),
            ),
            data::Ingredient::new(
                ...
                0,
                "Petrology",
                vec![(Property::DirectHealing, Modifier::new(None, None))]
                    .into_iter()
                    .collect(),
            ),
        ];
        ...
        let characters = vec![data::Character::new(
            "default",
            vec![
                ("Herbology".to_string(), 100u8),
                ("Petrology".to_string(), 100u8),
            ]
            .into_iter()
            .collect(),
            100,
    ...
    fn test_mix_dh() {
        ...
        let mix = Mix {
            alvarin_clade: true,
            advanced_potion_making_mod: 1.2,
            ingredients: reference.ingredients_from_names(ingredients()).unwrap(),
        };
    ...
    fn test_mix_effects() {
        ...
        assert!(approx_eq!(
            f64,
            actual[Property::DirectHealing as usize].inner(),
            3.25,
            epsilon = 0.01
        ));
    }
